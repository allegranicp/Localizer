...
vector< vector <float> > move(int dy, int dx, vector < vector <float> > beliefs, float blurring)
{
      ...
      for (int i = 0; i < height; ++i)
      {
          for (int j = 0; j < width; ++j)
          {
              new_i = mod((i + dy), width);
              new_j = mod((j + dx), height);
...
/**
    ...
    @return - a normalized two dimensional grid of floats 
    	   representing the updated beliefs for the robot. 
*/
vector< vector <float> > sense(char color, vector< vector <char> > grid, vector< vector <float> > beliefs, float p_hit,
	float p_miss) 
{
      vector < vector <float> > new_beliefs;
      vector < vector <float> > normalized_new_belief;
      vector <float> row_non_normalized;
      vector <float> row_normalized;
      float total_prob  = 0.0;
      float non_normalized_belief;
      float hit;
      ...
      for (int i = 0; i < grid.size(); ++i)
      {
          for (int j = 0; j < grid[0].size(); ++j)
          {
              ...
              total_prob += non_normalized_belief;
              row_non_normalized.push_back(non_normalized_belief);
          }
          
          new_beliefs.push_back(row_non_normalized);
          row_non_normalized;
      }
      
      for (int row = 0; row < new_beliefs.size(); ++row)
      {
          for (int col = 0; col < new_beliefs[0].size(); ++col)
          {
              row_normalized.push_back(new_beliefs[row][col]/total_prob);
          }
          
          normalized_new_belief.push_back(row_normalized);
          row_normalized;
      }
